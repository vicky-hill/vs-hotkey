...
const generateSnippet = async ({ type, prompt, resourceName, functionName, fileResourceName }) => {
    const template = templates_1.default.find(template => template.type === type && template.prompt === prompt);
    if (!template)
        return null;
    const snippetText = (0, string_1.replaceResource)(template.template, resourceName, functionName || null, fileResourceName || null);
